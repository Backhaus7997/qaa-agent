...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(planningDir)) {
    addIssue('error', 'E001', '.planning/ directory not found', 'Initialize project');
    output({
      status: 'broken',
      errors,
      warnings,
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Initialize project');
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Create a new milestone roadmap');
  }

  // --- Check 4: STATE.md exists and references valid phases ---
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run validate health --repair to regenerate', true);
    ...
    for (const ref of phaseRefs) {
      ...
      if (!diskPhases.has(ref) && !diskPhases.has(normalizedRef) && !diskPhases.has(String(parseInt(ref, 10)))) {
        ...
        if (diskPhases.size > 0) {
          addIssue('warning', 'W002', `STATE.md references phase ${ref}, but only phases ${[...diskPhases].sort().join(', ')} exist`, 'Run validate health --repair to regenerate STATE.md', true);
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run validate health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run validate health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run validate health --repair to add key', true);
    ...
    for (const e of phaseEntries) {
      ...
      if (hasResearch && !hasValidation) {
        ...
        if (researchContent.includes('## Validation Architecture')) {
          addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run planning with research to regenerate');
        }
      }
    }
  } catch {}
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by validate health --repair\n`;
